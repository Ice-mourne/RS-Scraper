...
import { FetchResult, ProxyObject } from './types.js'
...
const fetchText = async (url: string, tries: number = 3): Promise<string> => {
  try {
    return fetch(url)
      .then((res) => res.text())
...
const getProxyList = async () => {
  ...
  const getFromCheckerProxy = async (tries: number = 0) => {
    if (tries > 3) return []
    try {
      const resp = await fetch(`https://checkerproxy.net/api/archive/${currentDate}`)
      const json = (await resp.json()) as CheckerProxyResponse

      return json.flatMap((proxy) => {
        // allow only http and https
        if (proxy.type !== 1 && proxy.type !== 2) return []
        return proxy.addr
      })
    } catch (error) {
...
export async function getWorkingProxies() {
  ...
  let workingProxies: ProxyObject[] = []

  let responses: Promise<FetchResult>[] = []
  ...
  while (proxyMap.size > 0 && currentlyInQueue < MAX_REQUESTS) {
    ...
    promise.then(async (response) => {
      ...
      if (response.status === 'success') {
        ...
        if (response.proxy.failedCount >= 2) return
