...
for (const key in categories) {
  ...
  const response = fetch(`https://secure.runescape.com/m=itemdb_rs/catalogue?cat=${category.id}&page=1`).then((resp) =>
    resp.text()
  )
